<script lang="ts">
  ...
  let {
    ...
    // Items showing only their title row.
  ...
  const open = $derived(!collapsed.has(exercise));
...
<div class="card flex flex-col gap-4" data-exercise>
  <div class="flex flex-wrap items-center gap-2">
    <button
      ...
      onclick={() => (open ? collapsed.add(exercise) : collapsed.delete(exercise))}
      aria-expanded={open}
      aria-label={open ? "Collapse the exercise" : "Expand the exercise"}
      data-collapse
    >
    ...
    <span class="text-sm muted" data-chart-count>
      {#if !open}{describeDraft(exercise)} ·{/if}
      {detail.chartCount}
