class PokerRange {
  ...
  changeActionAtVector(vector: number[], action: Action) {
    const index = Math.floor(vector[0]) * 13 + Math.floor(vector[1]);
    this.range[index] = action;
  }

  changeActionAtVectors(vectors: number[][], action: Action) {
    for (const vector of vectors) {
      this.changeActionAtVector(vector, action);
    }
  }
...
function lineFromTwoHands(hand1: Hand, hand2: Hand): number[][] {
  ...
  const slope = (vec2[1] - vec1[1]) / (vec2[0] - vec1[0]);

  let x = vec1[0];
  while (x <= vec2[0]) {
    const y = slope * (x - vec1[0]) + vec1[1];
    range.push([Math.round(x), Math.round(y)]);
    x += 1;
  }
  return range;
}
...
function handToVector(hand: Hand): number[] {
  return [Math.floor(hand / 13), hand % 13];
}
